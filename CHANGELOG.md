...
### Added
- **NIP-89 kind-31990 client handler — adopted NostrHub-published event coordinate.** When PR #183 shipped, the `client` naddr1 baked into `scripts/fetch-ditto.mjs` and the `CLIENT_TAG` constant in `src/lib/routes/client.ts` both hardcoded the coordinate `31990:291c75d…:nostr-station` — aspirational, since the actual kind-31990 handler event hadn't been published yet. PR #185 had wired up `scripts/publish-client-handler.mjs` to broadcast the missing event with `NOSTR_STATION_NSEC` in the terminal, but exposing the project nsec in shell environment + history is a non-trivial security trade-off we'd rather not take. NostrHub's "Submit New App" form (nostrhub.io/apps/new) publishes kind-31990 via NIP-07 browser signing — nsec stays in the signer extension where it belongs — so we used their form instead. They auto-generated the d-tag as a random slug `wsfom06gac`, so the published coordinate is `31990:291c75d…:wsfom06gac` rather than the aspirational `nostr-station`. Updated both hardcoded references (CLIENT_TAG value + Ditto bundle's baked `client` naddr1) to point at the actual published coordinate, re-encoded the naddr1 with `wss://relay.damus.io` as the relay hint (general-purpose, replacing the inherited `wss://relay.nsite.lol` which was specific to nsite resolution), and rebuilt the Ditto bundle so the iframe-side tag matches the native-side tag. Removed `scripts/publish-client-handler.mjs` + its npm script entry — no longer needed since NostrHub owns the publish flow. The d-tag is an opaque internal identifier never shown to users; the rendered card (logo + name + about + #nostr-dev hashtag + "Open App" CTA pointing at the dedicated nsite landing page) is what NIP-89-aware clients display. If the event is ever deleted-and-recreated on NostrHub (rare — kind-31990 is replaceable so edits keep the same coordinate), the new d-tag must be updated in both files and `npm run update-ditto` re-run.
