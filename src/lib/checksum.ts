 ...
 *
 * Pulled into its own module so install.ts's binary-fetch helpers can stay
 * focused on the network/spawn flow, and the verify path can be unit-tested
 * without dragging execa + the cargo install machinery into the test process.
 *
...
/**
 * Computes SHA256 of `filePath`, returns hex digest in lowercase.
 * Caller is responsible for handling read errors — we don't catch them
 * here so a missing file surfaces as a thrown error rather than a fake
...
export function verifyFileSha256(filePath: string, expectedHex: string): boolean {
  ...
  // Length-stable XOR comparison — the inputs are public hex digests so
  // timing-attack risk is theoretical, but the cost of doing it right is
  // ~zero and avoids a future reviewer flagging the naive `===`.
  let diff = 0;
