...
export function communityRelayMetadataPath(communityDir: string): string {
  return path.join(communityDir, 'relay_metadata.json');
}

// =====================================================================
// relay_metadata.json — NIP-11 served by GRAIN
//
// GRAIN reads this file at startup and serves its contents on the NIP-11
// endpoint (`GET /` with Accept: application/nostr+json). If missing,
// GRAIN auto-generates a default file from an embedded template — which
// could include the community's friendly name, defeating the privacy
// model. We always pre-write this file with deliberately-opaque values
// so a mesh-member probing the port sees nothing identifying.
//
// The community's user-facing `name` and `description` live in
// community.json (dashboard-only). The fields here are NIP-11's public
// face — by default, an opaque relay identifier and an empty
// description. Users can override via the Settings tab if they don't
// care; the default is "expose nothing".

export interface RelayMetadata {
  name:         string;
  description:  string;
  pubkey?:      string;          // admin pubkey (hex)
  contact?:     string;          // optional; empty by default
  software?:    string;
  version?:     string;
...
/**
 * Default opaque NIP-11 payload. The `name` deliberately uses a
 * random hex tag rather than the community's friendly name so a
 * port-scanner-on-the-mesh learns nothing identifying. `description`
 * is empty for the same reason — the user must explicitly opt into
 * publishing one via Settings.
 */
export function defaultRelayMetadata(opts: { adminPubkey: string }): RelayMetadata {
  const tag = crypto.randomBytes(4).toString('hex');
  return {
    name:        `private-relay-${tag}`,
    description: '',
    pubkey:      opts.adminPubkey.toLowerCase(),
    contact:     '',
  };
}

export function readRelayMetadata(file: string): RelayMetadata | null {
...
export function writeRelayMetadata(file: string, meta: RelayMetadata): void {
  // JSON.stringify with 2-space indent so a moderator who opens the
  // file directly (e.g. to add a custom contact) gets a readable
  // diff. atomicWriteFileSync ensures GRAIN's startup read never
  // sees a torn file.
  atomicWriteFileSync(file, JSON.stringify(meta, null, 2) + '\n');
}
